...
import { downloadSupervisorWeekReportAPI } from '../../../lib/api';
...
function buildWeekLabel(activeWeek: WeekInfo | null) {
  ...
  if (!activeWeek.submitted_at) {
    return activeWeek.name;
  }
...
export default function WeeklyReportForm({ group, activeWeek }: WeeklyReportFormProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      const blob = await downloadSupervisorWeekReportAPI(group.id, activeWeek.id, form);
    ...
    <div className="mb-10 rounded-2xl border border-[#E8DDCC] bg-[#FCFAF7] p-6 shadow-sm">
      <div className="mb-6 flex flex-col gap-4 border-b border-[#E8DDCC] pb-6 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-gray-600">
            Generate the formal weekly progress report for the selected submission. The group and week are prefilled,
            and you can complete any missing academic details before downloading the PDF.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="text-sm font-semibold text-[#5B4636]">
          ...
          <input
            ...
            onChange={(event) => updateField('semesterSection', event.target.value)}
          />
        </label>
        <label className="text-sm font-semibold text-[#5B4636]">
          Faculty Supervisor
          <input
            ...
            onChange={(event) => updateField('facultySupervisor', event.target.value)}
          />
        </label>
        <label className="text-sm font-semibold text-[#5B4636]">
          Week / Date Label
          <input
            className="mt-2 w-full rounded-lg border border-[#E3D8CB] bg-white px-4 py-3 text-sm outline-none transition-colors focus:border-[#A06B40]"
            value={form.weekInfo}
            onChange={(event) => updateField('weekInfo', event.target.value)}
          />
        </label>
      </div>
          ...
          {form.students.map((student, index) => (
            <div key={`${student.email || student.name}-${index}`} className="rounded-xl border border-[#E8DDCC] bg-white p-4">
              ...
              <div className="grid gap-3">
                <input
                  className="rounded-lg border border-[#E3D8CB] bg-[#FCFAF7] px-3 py-2 text-sm outline-none transition-colors focus:border-[#A06B40]"
                  value={student.name}
                  onChange={(event) => updateStudentField(index, 'name', event.target.value)}
                  placeholder="Full name"
                />
                <input
                  className="rounded-lg border border-[#E3D8CB] bg-[#FCFAF7] px-3 py-2 text-sm outline-none transition-colors focus:border-[#A06B40]"
                  value={student.reg}
                  onChange={(event) => updateStudentField(index, 'reg', event.target.value)}
                  placeholder="Registration number"
                />
                <input
                  className="rounded-lg border border-[#E3D8CB] bg-[#FCFAF7] px-3 py-2 text-sm outline-none transition-colors focus:border-[#A06B40]"
                  value={student.mobile}
                  onChange={(event) => updateStudentField(index, 'mobile', event.target.value)}
                  placeholder="Mobile number"
                />
                <input
                  className="rounded-lg border border-[#E3D8CB] bg-[#FCFAF7] px-3 py-2 text-sm outline-none transition-colors focus:border-[#A06B40]"
                  value={student.email}
                  onChange={(event) => updateStudentField(index, 'email', event.target.value)}
      ...
      <div className="mt-6 grid gap-4">
        <label className="text-sm font-semibold text-[#5B4636]">
          ...
          <textarea
            className="mt-2 min-h-[130px] w-full rounded-xl border border-[#E3D8CB] bg-white px-4 py-3 text-sm outline-none transition-colors focus:border-[#A06B40]"
            value={form.weeklySummary}
            onChange={(event) => updateField('weeklySummary', event.target.value)}
            placeholder="Describe what the group completed this week."
          />
        </label>
        <label className="text-sm font-semibold text-[#5B4636]">
          ...
          <textarea
            ...
            onChange={(event) => updateField('individualContribution', event.target.value)}
            placeholder="Example: Aditi - model training, Rahul - UI implementation, ..."
          />
        </label>
      </div>
